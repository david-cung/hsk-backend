...
from app.practice_engine import canonical_type, evaluate_answer, public_question_config
    ...
    def finalize(self, db: Session, user: User, attempt: ExamAttempt, exam: MockTest, expired: bool = False) -> None:
        ...
        for row in rows:
            ...
            if question and row.answer is not None:
                ...
                normalized = evaluation.get("normalized_answer")
...
def _section_for_question(question: Question) -> str:
    qtype = canonical_type(question.question_type).value.upper()
...
def _correct_answer_for_question(question: Question | None) -> Any:
    if not question:
        return None
    try:
        return evaluate_answer(question, "").get("correct_answer")
